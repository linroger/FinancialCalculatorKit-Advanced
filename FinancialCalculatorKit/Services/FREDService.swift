...
class FREDService {
    ...
    init() {
        // Get API key from environment variables or use demo key with limited functionality
        if let envKey = ProcessInfo.processInfo.environment["FRED_API_KEY"], !envKey.isEmpty {
            self.apiKey = envKey
        } else {
            // Use publicly available demo key with rate limits for development
            // In production, this should be provided via secure configuration
            self.apiKey = "demo"
            print("Warning: Using demo FRED API key. Set FRED_API_KEY environment variable for full functionality.")
        }

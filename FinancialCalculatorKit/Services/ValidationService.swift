...
public final class ValidationService: ObservableObject {
    ...
    public func validateBatch(_ fields: [FieldValidationConfig]) async -> [String: FinancialValidation.ValidationResult] {
        await withTaskGroup(of: (String, FinancialValidation.ValidationResult).self) { group in
            for field in fields {
                group.addTask { [weak self] in
                    guard let self = self else { return (field.id, .invalid(errorType: .required, message: "Validation service unavailable")) }
                    
                    let result = await self.performAsyncValidation(
                        fieldId: field.id,
                        value: field.value,
                        rules: field.rules,
            ...
            for await (fieldId, result) in group {

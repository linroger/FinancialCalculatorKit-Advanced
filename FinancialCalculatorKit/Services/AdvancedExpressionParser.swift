...
class AdvancedExpressionParser {
    ...
    private func validateSyntax(_ expression: String) throws {
        ...
        for match in matches {
            ...
            let matchString = String(expression[matchRange])
    ...
    private func convertFunctionsToLatex(_ expression: String) -> String {
        ...
        if let factorialRegex = try? NSRegularExpression(pattern: factorialPattern) {
        ...
        let absPattern = #"\\left\|\(([^)]+)\)"#
        if let absRegex = try? NSRegularExpression(pattern: absPattern) {
    ...
    private func convertRootsToLatex(_ expression: String) -> String {
        ...
        if let cbrtRegex = try? NSRegularExpression(pattern: cbrtPattern) {
    ...
    private func convertSummationToLatex(_ expression: String) -> String {
        ...
        let sumPattern = #"sum\(([^,]+),([^,]+),([^)]+)\)"#
        guard let regex = try? NSRegularExpression(pattern: sumPattern) else {
            return expression
        }
        let range = NSRange(expression.startIndex..., in: expression)
    ...
    private func simplifyExpression(_ expression: String) -> String {
        ...
        if let regex = try? NSRegularExpression(pattern: singleNumPattern) {

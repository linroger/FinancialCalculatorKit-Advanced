...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $viewModel.showingCalculationSheet) {
            CalculationSheetView(viewModel: viewModel)
                .environment(viewModel)
        }
...
struct SidebarView: View {
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItemGroup(placement: .primaryAction) {
                ...
                Button(action: { openCompactCalculator() }) {
                    Image(systemName: "macwindow.badge.plus")
                }
                .help("Open compact calculator in new window")
    ...
    private func openCompactCalculator() {
        openWindow(id: "compact-calculator")
    }
}
...
struct DetailView: View {
    ...
    var body: some View {
        ...
        .responsiveFrame(minWidth: 500, idealWidth: 800, minHeight: 400, idealHeight: 600)
    }
}
...
struct CalculationSheetView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .padding()
            .navigationTitle(viewModel.selectedCalculation == nil ? "New Calculation" : "Edit Calculation")
            .onAppear {
                if let calculation = viewModel.selectedCalculation {
                    name = calculation.name
                    notes = calculation.notes
                    isFavorite = calculation.isFavorite
                    currency = calculation.currency
                } else {
                    // Set default currency for new calculations
                    currency = viewModel.userPreferences.defaultCurrency
                }
            }
            .toolbar {
    ...
    private func saveCalculation() {
        if let existingCalculation = viewModel.selectedCalculation {
            // Update existing calculation
            existingCalculation.name = name
            existingCalculation.notes = notes
            existingCalculation.isFavorite = isFavorite
            existingCalculation.currency = currency
            existingCalculation.updateTimestamp()
        } else {
            // Create new calculation
            let newCalculation = FinancialCalculation(
                name: name,
                calculationType: viewModel.selectedCalculationType,
                currency: currency,
                notes: notes
            )
            newCalculation.isFavorite = isFavorite
        ...
        do {
            ...
            viewModel.handleError(FinancialCalculatorError.dataImportFailed("Failed to save calculation: \(error.localizedDescription)"))
        }
    }
}

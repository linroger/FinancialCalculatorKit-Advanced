...
struct CompactCalculatorView: View {
    ...
    var body: some View {
        ...
        .responsivePadding()
        .responsiveFrame(minWidth: 600, idealWidth: 800, maxWidth: 1200, minHeight: 500, idealHeight: 600)
    ...
    private func calculateResults() {
        ...
        case .interestRate:
            // Use Newton-Raphson method for accurate interest rate calculation
            let calculatedRate = CalculationEngine.calculateInterestRate(
                presentValue: principal,
                futureValue: nil,
                payment: -monthlyPayment,
                numberOfPeriods: totalPayments,
        ...
        case .term:
            if monthlyRate == 0 {
                guard monthlyPayment > 0 else { result = 0; break }
                result = principal / monthlyPayment / 12.0
            } else {
                guard principal > 0, monthlyRate > 0, monthlyPayment > 0 else { result = 0; break }
    ...
    private func generateBalanceData() {
        ...
        let monthlyRate = interestRate / 100.0 / 12.0
        let totalPayments = termYears * 12.0
        // var remainingBalance = principal  // Will be used in full balance tracking implementation

...
struct LoanCalculatorView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            loadUserPreferences()
            // Check if the selected calculation is a loan calculation
            if let selectedCalc = mainViewModel.selectedCalculation as? LoanCalculation {
                calculation = selectedCalc
                principalAmount = selectedCalc.principalAmount
                annualInterestRate = selectedCalc.annualInterestRate
                loanTermYears = selectedCalc.loanTermYears
                downPayment = selectedCalc.downPayment
                extraPayment = selectedCalc.extraPayment
                paymentFrequency = selectedCalc.paymentFrequency
                loanType = selectedCalc.loanType
                currency = selectedCalc.currency

...
struct LoanResultView: View {
            ...
            HStack(spacing: 20) {
                VStack(alignment: .leading, spacing: 2) {
                    Text("Loan Amount")
                        .font(.caption)
                        .foregroundColor(.secondary)
                    Text(currency.formatValue(calculation.principalAmount - (calculation.downPayment ?? 0)))
                        .font(.callout)
                        .fontWeight(.semibold)
                }
    ...
    private var amortizationChartView: some View {
        if let amortizationSchedule = calculation.amortizationSchedule {
            AmortizationChart(
    ...
    private var paymentBreakdownView: some View {
        VStack(spacing: 16) {
            ...
            if let schedule = calculation.amortizationSchedule,
        ...
        VStack(spacing: 16) {
            ...
            if let schedule = calculation.amortizationSchedule {
                InterestAccumulationChart(
                    schedule: schedule,
                    currency: currency,
                    extraPayment: calculation.extraPayment ?? 0
                )
            }
    ...
    private func calculateSavingsWithExtraPayment(_ amount: Double) -> Double? {
        ...
        let principal = calculation.principalAmount - (calculation.downPayment ?? 0)
    ...
    private func calculateScenario(
        ...
        let extra = extraPayment ?? calculation.extraPayment ?? 0
    ...
    private func generateInsights() -> [String] {
        ...
        if let totalInterest = result.secondaryValues["Total Interest"],
           let principal = result.secondaryValues["Principal"] ?? (calculation.principalAmount - (calculation.downPayment ?? 0)) as Double? {
        ...
        if let extraPayment = calculation.extraPayment, extraPayment > 0,
        ...
        if let schedule = calculation.amortizationSchedule,
           let firstYear = schedule.prefix(calculation.paymentFrequency.periodsPerYear).reduce(0, { $0 + $1.interestPayment }),
           let firstYearPrincipal = schedule.prefix(calculation.paymentFrequency.periodsPerYear).reduce(0, { $0 + $1.principalPayment }) as Double? {
...
struct InterestAccumulationChart: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Chart {
                ForEach(schedule.filter { $0.period % max(1, schedule.count / 50) == 0 || $0.period == 1 || $0.period == schedule.count }) { entry in
                    LineMark(
                ...
                if extraPayment > 0, let lastEntry = schedule.last {
                    // Show potential interest without extra payments (rough estimate)
                    let baseInterest = lastEntry.cumulativeInterest * 1.15
                    ...
                    LineMark(
                        ...
                        y: .value("Without Extra", baseInterest)
                    )

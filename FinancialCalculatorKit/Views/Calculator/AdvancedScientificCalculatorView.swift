...
struct AdvancedScientificCalculatorView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            NavigationSplitView {
                sidebarContent
                    .frame(minWidth: 320, idealWidth: min(450, geometry.size.width * 0.35))
            } detail: {
        ...
        .responsiveFrame(minWidth: 1000, idealWidth: 1400, minHeight: 800, idealHeight: 1000)
    }
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    mainExpressionDisplay
                        .frame(minHeight: geometry.size.height > 800 ? 140 : 100)
                    ...
                    // Scientific keyboard - make it adaptive
                    if geometry.size.width > 800 {
                        scientificKeyboardSection
                    } else {
                        compactKeyboardSection
                    }
                }
                .padding(.horizontal, max(16, min(32, geometry.size.width * 0.04)))
                .padding(.vertical, 20)
            }
        }
    ...
    @ViewBuilder
    private var compactKeyboardSection: some View {
        VStack(spacing: 12) {
            // Essential functions in compact layout
            LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 6), spacing: 8) {
                ForEach(["sin", "cos", "tan", "log", "ln", "√x"], id: \.self) { symbol in
                    scientificKeyButton(symbol)
                        .frame(height: 36)
                }
            }

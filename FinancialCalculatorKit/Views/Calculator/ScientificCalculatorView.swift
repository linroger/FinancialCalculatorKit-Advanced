...
struct ScientificCalculatorView: View {
    ...
    private func calculateResult() {
        // Convert expression for angle mode
        let convertedExpression = convertAngleExpression(expression)
        
        // Use MathParser to evaluate
        let parser = MathParser()
        guard let evaluator = parser.parse(convertedExpression) else {
            result = "Error"
            return
        }
    ...
    private func convertAngleExpression(_ expr: String) -> String {
        var converted = expr
        
        // Convert trigonometric functions based on angle mode
        switch angleMode {
        case .degrees:
            converted = converted.replacingOccurrences(of: "sin(", with: "sin(deg2rad(")
            converted = converted.replacingOccurrences(of: "cos(", with: "cos(deg2rad(")
            converted = converted.replacingOccurrences(of: "tan(", with: "tan(deg2rad(")
        case .gradians:
            converted = converted.replacingOccurrences(of: "sin(", with: "sin(grad2rad(")
            converted = converted.replacingOccurrences(of: "cos(", with: "cos(grad2rad(")
            converted = converted.replacingOccurrences(of: "tan(", with: "tan(grad2rad(")
        case .radians:

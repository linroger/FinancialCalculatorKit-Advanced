...
// MARK: - AutoFill Formula Enum

enum AutoFillFormula: String, CaseIterable {
    case volumeOfSphere = "volume_of_sphere"
    case areaOfCircle = "area_of_circle"
    case pythagoreanTheorem = "pythagorean_theorem"
    case quadraticFormula = "quadratic_formula"
    case distanceFormula = "distance_formula"
...
struct SummationTemplateView: View {
    ...
    private var startValue: String {
        if let start = values["start"] {
            return String(format: "%.0f", start)
        }
        return "1"
    }
    
    private var endValue: String {
        if let end = values["end"] {
            return String(format: "%.0f", end)
        }
        return "n"
    }
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            if !values.isEmpty {
                LaTeX("$\\sum_{i=\(startValue)}^{\(endValue)} f(i)$")
                    .frame(height: 30)
            }
        }
...
struct IntegralTemplateView: View {
    ...
    private var lowerBound: String {
        if let lower = values["lower"] {
            return String(format: "%.1f", lower)
        }
        return "a"
    }
    
    private var upperBound: String {
        if let upper = values["upper"] {
            return String(format: "%.1f", upper)
        }
        return "b"
    }
        ...
        VStack(spacing: 16) {
            HStack(spacing: 12) {
                VStack(spacing: 0) {
                    ...
                    Text("∫")
                        .font(.system(size: 48, weight: .light))
                        .foregroundColor(.financialGreen)
                    Text(lowerBound)
                        .font(.financialNumberSmall)
                }
            ...
            if !values.isEmpty {
                LaTeX("$\\int_{\(lowerBound)}^{\(upperBound)} f(x) \\, dx$")
                    .frame(height: 30)
            }
        }
        .foregroundColor(.primary)
    ...
    var body: some View {
        VStack(spacing: 16) {
            VStack(spacing: 4) {
                ...
                Rectangle()
                    .frame(height: 2)
                    .frame(width: max(40, CGFloat(max(numerator.count, denominator.count)) * 12))
                    .foregroundColor(.primary)
    ...
    var body: some View {
        VStack(spacing: 16) {
            HStack(alignment: .bottom, spacing: 2) {
                ...
                Text(exponent)
                    .font(.financialNumberSmall)
                    .foregroundColor(.financialOrange)
                    .offset(y: -8)
            }
...
struct RootTemplateView: View {
    ...
    private var index: String {
        if let idx = values["index"] {
            return String(format: "%.0f", idx)
        }
        return "n"
    }

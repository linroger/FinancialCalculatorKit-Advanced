...
struct InvestmentCalculatorView: View {
    ...
    // MIRR parameters
    @State private var financeRate: Double = 8.0
    @State private var reinvestmentRate: Double = 12.0
    
    // Advanced IRR parameters
    @State private var useHighPrecision: Bool = false
        ...
        VStack(spacing: 20) {
            ...
            GroupBox("Investment Details") {
                VStack(spacing: 16) {
                    ...
                    // MIRR Parameters (conditional)
                    if analysisType == .mirr {
                        VStack(alignment: .leading, spacing: 8) {
                            Text("MIRR Parameters")
                                .font(.financialSubheadline)
                                .fontWeight(.medium)
                            ...
                            VStack(spacing: 12) {
                                PercentageInputField(
                                    ...
                                    value: Binding(
                                        get: { financeRate },
                                        set: { if let newValue = $0 { financeRate = max(0, newValue) } }
                                    ),
                                    isRequired: true,
                                    helpText: "The cost of borrowing/financing for the investment"
                                )
                                
                                PercentageInputField(
                                    title: "Reinvestment Rate",
                                    value: Binding(
                                        get: { reinvestmentRate },
                                        set: { if let newValue = $0 { reinvestmentRate = max(0, newValue) } }
                                    ),
                                    isRequired: true,
                                    helpText: "The rate at which positive cash flows can be reinvested"
                                )
                            }
                        }
                        .padding(.top, 8)
                    ...
                    // Advanced IRR Options (conditional)
                    if analysisType == .advancedIRR {
                        VStack(alignment: .leading, spacing: 8) {
                            Text("Advanced IRR Options")
                                .font(.financialSubheadline)
                                .fontWeight(.medium)
    ...
    private var canSave: Bool {
        calculationResult?.isValid == true
    }
    ...
    private func performCalculation() {
        ...
        let calculationName = "Investment Analysis - \(formatter.string(from: Date()))"
        ...
        tempCalculation.financeRate = financeRate
        tempCalculation.reinvestmentRate = reinvestmentRate
        tempCalculation.useHighPrecision = useHighPrecision
    ...
    private func saveCalculation() {
        ...
        // Set additional parameters
        investmentCalculation.financeRate = financeRate
        investmentCalculation.reinvestmentRate = reinvestmentRate
        investmentCalculation.useHighPrecision = useHighPrecision
    ...
    private func formatSecondaryValue(key: String, value: Double) -> String {
        ...
        } else if key == "Calculation Method" {
            return value == 1.0 ? "Newton-Raphson" : "Decimal Search"
        } else if key == "Iterations" {
            return "\(Int(value)) iterations"
        } else if key == "Convergence Confidence" {
            return value == 3.0 ? "High" : (value == 2.0 ? "Medium" : "Low")
        } else if key == "Multiple IRRs Detected" {
            return "\(Int(value)) IRR(s) found"
        } else if key == "Investment Rounds" {
            return "\(Int(value)) round(s)"
        } else {
    ...
    private func generateInsights() -> [String] {
        ...
        if analysisType == .irr || analysisType == .both || analysisType == .advancedIRR || analysisType == .mirr || analysisType == .blendedIRR {
        ...
        // Advanced IRR specific insights
        if analysisType == .advancedIRR {
            if let convergenceConfidence = result.secondaryValues["Convergence Confidence"] {
                if convergenceConfidence == 3.0 {
                    insights.append("High confidence in IRR calculation using advanced Newton-Raphson method")
                } else if convergenceConfidence == 1.0 {
                    insights.append("Low confidence - consider reviewing cash flow assumptions")
                }
            }
            
            if let multipleIRRs = result.secondaryValues["Multiple IRRs Detected"], multipleIRRs > 1 {
                insights.append("Warning: Multiple IRRs detected - investment has unconventional cash flow pattern")
            }
            
            if let iterations = result.secondaryValues["Iterations"], iterations > 500 {
                insights.append("Complex cash flow pattern required many iterations to converge")
            }
        }
        
        // MIRR specific insights
        if analysisType == .mirr {
            if let traditionalIRR = result.secondaryValues["Traditional IRR"] {
                let mirrValue = result.primaryValue
                if mirrValue < traditionalIRR - 2 {
                    insights.append("MIRR is significantly lower than traditional IRR due to reinvestment assumptions")
                } else if mirrValue > traditionalIRR + 2 {
                    insights.append("MIRR is higher than traditional IRR - favorable reinvestment environment")
                } else {
                    insights.append("MIRR and traditional IRR are similar - reinvestment assumptions have minimal impact")
                }
            }
        }
...
struct ScenarioAnalysisView: View {
    ...
    @ViewBuilder
    private func npvColumn(for scenario: InvestmentScenarioResult) -> some View {
    ...
    @ViewBuilder
    private func irrColumn(for scenario: InvestmentScenarioResult) -> some View {
        Text("\(scenario.irr, specifier: "%.2f")%")
            .font(.financialNumber)
    }
    
    @ViewBuilder
    private func decisionColumn(for scenario: InvestmentScenarioResult) -> some View {
    ...
    private func generateScenarios() -> [InvestmentScenarioResult] {
        let baseCashFlows = investmentData.cashFlows
        var scenarios: [InvestmentScenarioResult] = []
    ...
    private func calculateScenario(name: String, cashFlows: [Double]) -> InvestmentScenarioResult {
        var allCashFlows = [-abs(investmentData.initialInvestment)]
        allCashFlows.append(contentsOf: cashFlows)
        ...
        return InvestmentScenarioResult(name: name, npv: npv, irr: irr)
    }
}
...
struct InvestmentScenarioResult: Identifiable {
    let id = UUID()
    let name: String
    let npv: Double
    let irr: Double

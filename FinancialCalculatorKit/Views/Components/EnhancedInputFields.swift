...
public struct EnhancedCurrencyInputField: View {
    ...
    private var currencyTextField: some View {
        let baseField = TextField(placeholder, text: $stringValue)
        ...
        let fieldWithAccessibility = baseField
            .accessibilityLabel(accessibilityLabel ?? "\(title) input field")
            .accessibilityHint(accessibilityHint ?? "Enter \(title) in \(currency.displayName)")

        let fieldWithHandlers = fieldWithAccessibility
            .onSubmit {
                validateAndFormat()
            }
...
public struct EnhancedPercentageInputField: View {
    ...
    public init(
        ...
        let formatter = Formatters.percentageFormatter
        formatter.minimumFractionDigits = 0

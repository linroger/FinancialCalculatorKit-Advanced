...
/// Enhanced detail row component for displaying key-value pairs with improved styling
struct DetailRow: View {
    ...
    enum DetailRowStyle {
        ...
        var valueFont: Font {
            switch self {
            case .prominent: return .system(.title3, design: .monospaced, weight: .semibold)
            case .currency, .percentage: return .system(.body, design: .monospaced, weight: .medium)
        ...
        var valueColor: Color {
            ...
            case .currency: return .financialGreen
            case .percentage: return .financialBlue
            default: return .secondary
            }
        }
    }
    
    init(title: String, value: String, subtitle: String? = nil, icon: String? = nil, style: DetailRowStyle = .standard, isHighlighted: Bool = false) {
        self.title = title
        self.value = value
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 12) {
            ...
            Text(value)
                ...
                .foregroundColor(isHighlighted ? .accentColor : style.valueColor)
                .multilineTextAlignment(.trailing)
        }
        .padding(.horizontal, 16)
        .padding(.vertical, style == .prominent ? 12 : 8)
        .background(
            Group {
                if isHighlighted {
...
/// Status indicator component for showing success, warning, error states
struct StatusIndicator: View {
    ...
    enum Status {
        ...
        var color: Color {
            switch self {
            case .success: return .financialGreen
            case .warning: return .financialOrange
            case .error: return .financialRed
            case .info: return .financialBlue
            }
        }
    ...
    ScrollView {
        VStack(spacing: 32) {
            ...
            GroupBox("Standard Financial Input") {
                VStack(spacing: 16) {
                    DetailRow(title: "Principal Amount", value: "$100,000.00", icon: "dollarsign.circle", style: .currency)
                    DetailRow(title: "Interest Rate", value: "5.50%", icon: "percent", style: .percentage, isHighlighted: true)
                    DetailRow(title: "Term", value: "30 years", icon: "calendar", style: .time)
                    DetailRow(title: "Monthly Payment", value: "$568.23", icon: "creditcard", style: .prominent)
                }
            }
            ...
            // Status Examples
            VStack(spacing: 12) {
                StatusIndicator(status: .success, message: "Calculation completed successfully")
                StatusIndicator(status: .warning, message: "Some inputs may be out of typical range")
                StatusIndicator(status: .error, message: "Unable to process calculation")
                StatusIndicator(status: .info, message: "Tip: Higher frequency payments reduce total interest")
            }
